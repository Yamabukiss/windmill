    ...
    void Windmill::drawBboxes(const cv::Mat &bgr, const std::vector<BoxInfo> &bboxes) {
        ...
        if (!bboxes.empty()) {
            for (size_t i = 0; i < bboxes.size(); i++) {
                ...
                cv::Point2f center((bbox.x1 + bbox.x2 + bbox.x3 + bbox.x4) / 4 * width_ratio,
...
//                        cv::putText(bgr,label_array[bbox.label],cv::Point2f(std::max(float(0),points_vec[1].x-10),std::max(points_vec[1].y-10,float(0))),cv::FONT_HERSHEY_SCRIPT_SIMPLEX,1,color,2);
    ...
    void Windmill::cvProcess(const cv::Mat &image) {
        ...
        if (red_)
            cv::inRange(hsv, cv::Scalar(red_lower_hsv_h_, red_lower_hsv_s_, red_lower_hsv_v_),
                        cv::Scalar(red_upper_hsv_h_, red_upper_hsv_s_, red_upper_hsv_v_), threshold);
        else
            cv::inRange(hsv, cv::Scalar(blue_lower_hsv_h_, blue_lower_hsv_s_, blue_lower_hsv_v_),
                        cv::Scalar(blue_upper_hsv_h_, blue_upper_hsv_s_, blue_upper_hsv_v_), threshold);
